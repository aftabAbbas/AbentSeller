...
class VHFileReceiver(itemView: View) : RecyclerView.ViewHolder(itemView) {
    ...
    fun setData(
        ...
        // Checking file if exists then hide download button
        val file = File(Constants.FILEPATH + arrayList[position].msg)
        if (file.exists()) {
            ivDownload.visibility = View.GONE
        } else {
        ...
        override fun doInBackground(vararg f_url: String?): String? {
            ...
            try {
                ...
                while (input.read(data).also { count = it } != -1) {
                    ...
                    (output as FileOutputStream).write(data, 0, count)
                }
                output.flush()
                output.close()
                input.close()
        ...
        override fun onProgressUpdate(vararg progress: String?) {
            Handler(Looper.getMainLooper()).post {
                ...
                if (progress[0]!!.toInt() == 100) {
                    Toast.makeText(
                        context,
                        context.resources.getString(R.string.download_completed),
                        Toast.LENGTH_SHORT
                    ).show()
                    clProgress.visibility = View.GONE
                }
